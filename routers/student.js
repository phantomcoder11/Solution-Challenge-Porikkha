...
const client = new OAuth2Client(process.env.OUTHTOKEN);
...
router.post( '/login' , ( req , res )=> {
    ...
    client.verifyIdToken({ idToken : tokenId , audience : process.env.OUTHTOKEN }).then((response)=>{
        ...
        if(email_verified){
            ...
            const student = { email_verified , email , name  };

            res.json(student);
        }
        else{
           
            res.json({ email_verified  , msg : 'Email not verified' });
        }
    })
})
